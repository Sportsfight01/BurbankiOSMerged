//
//  HomeLandPushNotifiactionVC.swift
//  BurbankApp
//
//  Created by Naveen Kourampalli on 23/07/25.
...
//
    ...
    class HomeLandPushNotifiactionVC: HeaderVC {
...
//            }
    ...
    //        Analytics.logEvent("BB_HomeAndLand_NameAndSize_", parameters: ["name" : self.homeLand?.houseName ?? "","size" : self.homeLand?.houseSize ?? "","jobID" : appDelegate.currentUser?.jobNumber ?? "0"])
            let homeLandPackageDetails = self.homeLandPackageDetails?.packageDetails
            self.lBHouseName.text = (homeLandPackageDetails?.houseName ?? "") + " " + "\(homeLandPackageDetails?.houseSize ?? 0)"
          ...
          self.lBAddress.text = self.homeLand?.address?.replacingOccurrences(of: " ,", with: ",").replacingOccurrences(of: " ,", with: ",").replacingOccurrences(of: " ,", with: ",").replacingOccurrences(of: " ,", with: ",") //replace " ," with "," until we remove 4 spaces infront of ","
            ...
            self.lBPrice.text = String.currencyFormate(Int32("\(homeLandPackageDetails?.price ?? 0)") ?? 0)
            self.lBBedrooms.text = "\(homeLandPackageDetails?.bedRooms ?? 0)"
            self.lBBathrooms.text = "\(homeLandPackageDetails?.bathRooms ?? 0)"
            self.lBParking.text = "\(homeLandPackageDetails?.carSpace ?? 0)"
            self.lBLandSize.text = "\(homeLandPackageDetails?.landSizeSqm ?? 0.0)"
            ...
            let tex = "LAND SIZE " + "\(homeLandPackageDetails?.landSizeSqm ?? 0.0)" + "m2"
            
            let mainStr = NSMutableAttributedString(string: "LAND SIZE ", attributes: [.font : FONT_LABEL_SUB_HEADING(size: 10)])
            let landSizeStr = NSAttributedString(string: "\(homeLandPackageDetails?.landSizeSqm ?? 0.0)m", attributes: [.font : FONT_LABEL_HEADING(size: 12)])
    ...
    //        lBLandSize.attributedText = attString
            ...
            self.addBreadCrumb(from: (homeLandPackageDetails?.houseName ?? "") + " " + "\(homeLandPackageDetails?.houseSize ?? 0)")

            
            self.btnFavorite.tintColor = APPCOLORS_3.GreyTextFont
            if Int(kUserID)! > 0 {// Current User
                self.btnFavorite.setBackgroundImage(homeLandPackageDetails?.isFav == true ? imageFavorite : imageUNFavorite, for: .normal)
                self.btnSaveDesign.backgroundColor = homeLandPackageDetails?.isFav == true ? APPCOLORS_3.LightGreyDisabled_BG : APPCOLORS_3.Orange_BG
    ...
    //            }
           ...
           // self.btnSaveDesign.isHidden = self.btnFavorite.isHidden
            ...
            self.lBEstateFacadeName.text = homeLandPackageDetails?.estateName ?? ""
            self.lBEstateAddress.text = homeLandPackageDetails?.address ?? ""

            
            self.imageHouse?.image = imageEmpty
            activity.startAnimating()

            if let imageURL = homeLandPackageDetails?.facadePermanentURL {
            ...
            if let packageDetails = self.homeLandPackageDetails {
                ...
                mapViewGoogle.setMapPosition(with: self.homeLand, zoomlevel: 12.0)
                
                mapViewGoogle.addMarkersForHomeLandPushNotifications(packages: [packageDetails])
            }
    ...
    extension HomeLandPushNotifiactionVC: ChildVCDelegate, UIScrollViewDelegate
    {
    ...
    extension HomeLandPushNotifiactionVC: CLLocationManagerDelegate, MKMapViewDelegate, GMSMapViewDelegate {
    ...
    extension HomeLandPushNotifiactionVC {
