//
//  HomeDesignsAndDisplayHomeDetailsVC.swift
//  BurbankApp
//
//  Created by Naveen Kourampalli on 16/07/25.
...
class HomeDesignsAndDisplayHomeDetailsVC: HeaderVC {
    ...
    //    @IBOutlet weak var nextDesignBTN: UIButton!
      ...
      var validFacadeNamesArray = [String]()
        ...
        var homeDesignData: HomeDesigns?
    ...
    var pushnotificatonsDetails : pushNtfcnModelInfo?
        ...
        override func viewDidLoad() {
            ...
            if pushnotificatonsDetails != nil{
                appDelegate.userData?.user?.userDetails?.userStateId = pushnotificatonsDetails?.stateId ?? ""
                fillAllDetails ()
                self.addBreadCrumb (from: (pushnotificatonsDetails?.houseName ?? "") + " " + (pushnotificatonsDetails?.houseSize ?? ""))
...
//            }else {
                addHeaderOptions(sort: false, map: false, favourites: true, howWorks: false, reset: false,totalCount: false,  delegate: self)
//            }
...
//            }
    ...
    //      validFacadeNamesArray = homeDesignDetails?.lsthouses?.validFacades?.components(separatedBy: "|")
            if let imageurl = homeDesignData?.facadePermanentUrl {
           ...
           // self.btnSaveDesign.isHidden = self.btnFavorite.isHidden
            ...
            if let subViews3D = self.btnMyPlace.superview?.subviews {
                for vi in subViews3D {
                    vi.isHidden = !(self.homeDesignData?.visualisation ?? false)
                }
            }
...
//        func fillDetailsFromNotifications(){
//            //      validFacadeNamesArray = homeDesignDetails?.lsthouses?.validFacades?.components(separatedBy: "|")
//                    if let imageurl = homeDesignData?.facadePermanentUrl {
//
//                        arrScrollImageUrls.removeAll()
...
//                }
    ...
    //            self.previousDesignBTN.isHidden = false
                homeDesignData = arrHomeDesignsDetails[selectedDesignCount - 1]
    ...
    //            self.nextDesignBTN.isHidden = false
        ...
        @IBAction func handleMyPlaceButton (_ sender: UIButton) {
            ...
            guard let url = URL(string:"\(My_Place3DBASEURL)/\(state)/myplace3dmobile/housename.\(self.homeDesignData?.houseName ?? "");housesize.\(self.homeDesignData?.houseSize ?? "0")/") else { return }
...
//            appDelegate.userData?.user?.userDetails?.userState = pushnotificatonsDetails?.state ?? ""
//            appDelegate.userData?.user?.userDetails?.userStateId = pushnotificatonsDetails?.stateId ?? ""
        ...
        @IBAction func handleFavoriteButtonAction (_ sender: UIButton) {
            ...
            if noNeedofGuestUserToast(self, message: "Please login to add favourites") {
                ...
                self.makeHomeDesignFavorite(!(homeDesignData!.isFav), homeDesignData!) { (success) in
                    if success {
                        
                        if (!(self.homeDesignData!.isFav) == true) {
                        ...
                        self.homeDesignData!.isFav = !(self.homeDesignData!.isFav)
                        
//                        self.btnFavorite.setImage(self.homeDesignData!.isFav == true ? imageFavorite : imageUNFavorite, for: .normal)
                        
                        self.btnSaveDesign.isHidden = true
                       ...
                       // if self.homeDesign!.favouritedUser?.userID == kUserID {
                        ...
                        updateHomeDesignsFavouritesCount(self.homeDesignData!.isFav == true)
                        //}
                    }
                }
            }
...
//                    self.handleFavoriteButtonAction(btnFavorite)
    ...
    extension HomeDesignsAndDisplayHomeDetailsVC: UITableViewDelegate, UITableViewDataSource {
    ...
    extension HomeDesignsAndDisplayHomeDetailsVC: ChildVCDelegate, RegionVCDelegate
    {
    ...
    extension HomeDesignsAndDisplayHomeDetailsVC {
        ...
        func pageUISetup () {
            ...
            setAppearanceFor(view: lBLot, backgroundColor: COLOR_CLEAR, textColor: APPCOLORS_3.GreyTextFont, textFont: FONT_LABEL_LIGHT(size: FONT_5))
    ...
    //        lBHomeLand.text = "House & Land"
        ...
        func getDesignDetailsFromPushNotification (_ design: pushNtfcnModelInfo, completion : @escaping (HomeDesignDetails) -> Void ) {
            
            _ = Networking.shared.GET_request(url: ServiceAPI.shared.URL_HomeDesignDetails(design.stateId, design.houseName, design.houseSize), userInfo: nil, success: { (data, response) in
                    ...
                    if let _ = result.value(forKey: "status"), (result.value(forKey: "status") as? Bool) == true {
                        ...
                        if let homeLandPackages = result.value(forKey: "getpackagebyName") as? [NSDictionary] {
                            ...
                            if arrHomeLandPackages.count > 0 {
                                ...
                                homeLand.design = self.homeDesignData
