    ...
    func setupLocalStorageForNotification(completion :@escaping((Int) -> ()))
    {
        ...
        APIManager.shared.getUserSelectedNotificationTypes { notificationTypes in
            ...
            let group = DispatchGroup()
                ...
                group.enter()
                APIManager.shared.getJobStepsList { result in
                    defer { group.leave() }
                    switch result
                    {
                    case .success(let progressArray):
                        DispatchQueue.main.async {
                            ...
                            print("-------------------- serviceCompletedStages : ", serviceCompletedStages.count)
                ...
                //add photos
                group.enter()
            APIManager.shared.getMyDocumentsAndPhotosForNotificationsAPI { servicePhotosArray in
                    defer { group.leave() }
                ...
                case .success(let notes):
                    DispatchQueue.main.async {
                        ...
                        if documentList.count > localStoragePhotos.count
                        {
                            for serviceItem in documentList
                            {
                                if localStoragePhotos.contains(where: { $0.photoURl == serviceItem.url }) { continue }
                                else {
                                    self.createAndAddRealmObject(taskName: "", taskId: 0, photoURL: serviceItem.url ?? "no url", date: serviceItem.metaData.date, isPhoto: true)
                                }
                            }
                        }
